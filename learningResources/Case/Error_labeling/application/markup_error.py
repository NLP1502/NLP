...
class DiffExtend:
	...
	def seg_label(source_seg, target_seg):
		seg_list_s = []
		seg_list_t = []
		index_s = index_t = 0
		for ss in source_seg.split(' '):
			seg_list_s.append((index_s, index_s + len(ss)))
			index_s += len(ss)
		for ts in target_seg.split(' '):
			seg_list_t.append((index_t, index_t + len(ts)))
			index_t += len(ts)
		de = DiffExtend()
		return de._label(source_seg.replace(' ', ''), target_seg.replace(' ', ''),
			seg_list=(seg_list_s, seg_list_t))
	...
	def _label(self, source, target, seg_list=None):
		...
		if seg_list:
			seg_list_s, seg_list_t = seg_list
		else:
...
if __name__ == '__main__':
	...
	parser.add_argument('--raw', '-r', help="file with probably erroneous sentences, one sentence a line")
	parser.add_argument('--ref', '-f', help="file with correct references corresponding to each in raw_file")
	parser.add_argument('--xml', '-x', help="file to save output infomation")
	parser.add_argument('--error_ratio', '-e', help="ratio over average number of errors within a raw line")
	parser.add_argument('--lenth_ratio', '-l', help="ratio over average short length divided by long length")
	parser.add_argument('--error_format', '-o', help="format of error info in output documents")
	parser.add_argument('--docum_format', '-d', help="format of output documents")
	parser.add_argument('--sample', '-s', help="sample ratio from origin output")
	parser.add_argument('--size', '-z', help="size of data to be disposed")
	args = parser.parse_args()
	
	error_ratio = 1.5 if not args.error_ratio else float(args.error_ratio)
	lenth_ratio = 1.5 if not args.lenth_ratio else float(args.lenth_ratio)
	raw_file_name = "NSMT\\hyp0.zh-en-zh.valid.txt" if not args.raw else (args.raw).replace('\\', '\\\\')
	ref_file_name = "NSMT\\ref.en-zh.valid.txt" if not args.ref else (args.ref).replace('\\', '\\\\')
	xml_file_name = "NSMT\\train.release.xml" if not args.xml else (args.xml).replace('\\', '\\\\')
	docum_format = '<DOC>\n<TEXT id="{:0>24}">\n{}\n</TEXT>\n<CORRECTION>\n{}\n</CORRECTION>\n{}</DOC>\n' \
		if not args.docum_format else args.docum_format
	error_format = '<ERROR start_off="{}"  end_off="{}"  type="{}"></ERROR>\n' \
		if not args.error_format else args.error_format
	sample_ratio = 0.01 if not args.sample else float(args.sample)
	batch_size = -1 if not args.size else int(args.size)

	long_len = short_len = 0
	error_sum = sentence_sum = 0
	wrongs, refers, oprats = [], [], []

	with open(raw_file_name, 'r', encoding="utf-8") as raw:
		wrongs = [line.strip() for line in raw.readlines()[: batch_size]]
	with open(ref_file_name, 'r', encoding="utf-8") as ref:
		refers = [line.strip() for line in ref.readlines()[: batch_size]]

	for wrong, refer in zip(wrongs, refers):
		tmp = DiffExtend.seg_label(wrong, refer)
		oprats.append(tmp)
		error_sum += len(tmp)
		sentence_sum += 1
		tmp_w = len(wrong.replace(' ', ''))
		tmp_r = len(refer.replace(' ', ''))
		long_len += max(tmp_w, tmp_r)
		short_len += min(tmp_w, tmp_r)
		if sentence_sum % 500 == 499:
			ratio = sentence_sum / len(wrongs)
			print("Progress: [{: <80}{:.0f}%]".format("#" * int(ratio * 80), ratio * 100), end="\r")

	error_threshold = error_sum * error_ratio / sentence_sum if sentence_sum != 0 else 5
	lenth_threshold = long_len * lenth_ratio / short_len if short_len != 0 else 0

	print()
	print("error_threshold %.2f" % error_threshold)
	print("lenth_threshold %.2f" % lenth_threshold)

	id = 0
	with open(xml_file_name, 'w', encoding="utf-8") as xml:
		with open(xml_file_name + ".sample", 'w', encoding="utf-8") as sample:
			for wrong, refer, oprat in zip(wrongs, refers, oprats):
				if len(oprat) > error_threshold:
					continue
				wrong = wrong.replace(' ', '')
				refer = refer.replace(' ', '')
				if max(len(wrong), len(refer)) / min(len(wrong), len(refer)) > lenth_threshold:
					continue
				error = ""
				for opt in oprat:
					error += error_format.format(opt[1], opt[2], transOpt(opt[0]))
				docum = docum_format.format(id, wrong, refer, error)
				xml.write(docum)
