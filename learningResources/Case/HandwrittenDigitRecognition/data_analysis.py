...
# 图像文件
images_idx3_ubyte_file = 'MNIST/raw/t10k-images.idx3-ubyte'
# 标签文件
labels_idx1_ubyte_file = 'MNIST/raw/t10k-labels.idx1-ubyte'
# 数据使用MNIST的测试集，共10000条数据
...
def load_data():
    ...
    # CNN的输入要求特征为Float32，标签为Long，这里很重要
    images = images.astype(np.float32)
    labels = labels.astype(np.int64)

    div = int(images.shape[0] * 0.8)  # 取前80%作为训练集，后20%作为测试集
    return images[: div], labels[: div], images[div:], labels[div:]


if __name__ == '__main__':  # 查看数据集加载效果
