...
class Config(object):
    ...
    question_train_set_items = 649632
    '''
    ...
    # vocab_size = 7073 # num of chars
    vocab_size = 138223  # num of words
    ...
    embedding_path = 'char_embedding_enhanceA.npz'  # Embedding
    train_data_path = 'train_enhanceA.npz'  # train
    labels_path = 'labels_enhanceA.json'  # labels
    test_data_path = 'test_enhanceA.npz'  # test
    ...
    model_num = 7
    data_root = ""
    labels_file = "labels_enhanceA.json"
    val = "val_enhanceA.npz"
...
def parse(self, kwargs, print_=False):
    ...
    if self.gpu1:
        self.train_data_path = 'train_enhanceA.npz'
        self.test_data_path = '%s.npz' % ('val_enhanceA' if self.val else 'test_enhanceA')
        self.labels_path = 'labels_enhanceA.json'
    ...
    if self.floyd:
        ...
        self.test_data_path = '%s.npz' % ('val_enhanceA' if self.val else 'test_enhanceA')
        self.labels_path = 'labels_enhanceA.json'
        self.embedding_path = 'char_embedding_enhanceA.npz'
    if self.zhuge:
        self.train_data_path = 'train_enhanceA.npz'
        self.test_data_path = '%s.npz' % ('val_enhanceA' if self.val else 'test_enhanceA')
        self.labels_path = 'labels_enhanceA.json'
    ...
    ### word和char的长度不一样 ##
    if self.type_ == 'word':
        self.vocab_size = 138223  # num of words
    ...
    if self.type_ == 'char':
        self.vocab_size = 7072  # num of words
