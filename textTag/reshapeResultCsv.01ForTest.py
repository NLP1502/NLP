...
# 修正上一版本在标签中，用,替换|导致后续无法正确切割含带逗的问题的标签
def main(**kwargs):
    infile = ""
    inEnhancement = ''
    questionfile = ""
    ...
    with open(infile, 'r', encoding='utf8') as fin, open(questionfile, 'w', encoding='utf8') as ftrain, open(topicfile, 'w', encoding='utf8') as ftopic:
        ...
        for i in process_bar:
            ...
            if changeZhHans:
                seg_list = jieba.cut(Converter('zh-hans').convert(data[i][1]).replace('\t', ''), cut_all=False)
            else:
            ...
            if changeZhHans:
                seg_list = jieba.cut(Converter('zh-hans').convert(data[i][2]).replace('\t', ''), cut_all=False)
            else:
            ...
            #         output_line2 += addLable + '|' + data[i][3].replace('\t', '') + '\n'
            #     else:
            #         output_line2 += data[i][3].replace('\t', '') + '|' + addLable + '\n'
            # else:
            #     output_line2 += data[i][3].replace('\t','') + '\n'
