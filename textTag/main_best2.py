#coding:utf8
from config_best2 import opt
import models
import os
import tqdm
from dataset_best import ZhihuData
import torch as t
import time
import fire
    ...
    for epoch in range(opt.max_epoch):
        ...
        for ii,((title,content),label) in tqdm.tqdm(enumerate(dataloader)):
            ...
            if ii%opt.plot_every ==opt.plot_every-1:
                ...
                print('prec:%s,recall:%s,score:%s,a:%s' %(prec_,recall_,score_,_ss))
                print('scores', score_meter.value()[0])
            ...
            if ii%opt.decay_every == opt.decay_every-1:   
                ...
                if scores>best_score:
                    print('saving')
                    best_score = scores
                    best_path = model.save(name = str(scores)+'&'+str(epoch)+'&'+str(ii),new=True)

                else:
                    try:
                        print('saving')
                        model.save(name=str(scores)+'&'+str(epoch)+'&'+str(ii), new=True)
