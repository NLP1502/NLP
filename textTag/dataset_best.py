...
class StackData(data.Dataset):
    def __init__(self,data_root,labels_file,val):
        ...
        self.train_data=t.zeros(self.all_num-self.val_num,25556*self.model_num)
        self.val_data=t.zeros(self.val_num,25556*self.model_num)
        for i in range(self.model_num):
            tmpdata=t.load(self.data_files_path[i]).float()
            self.train_data[:,i*25556:i*25556+25556]=tmpdata[:self.all_num-self.val_num]
            self.val_data[:,i*25556:i*25556+25556]=tmpdata[-1*self.val_num:]
...
class ZhihuData(data.Dataset):
    ...
    def __init__(self,train_root,labels_file,type_='char',augument=True):
        ...
        self.train_data = all_data_title[:-50000],all_data_content[:-50000]
        self.val_data = all_data_title[-50000:],all_data_content[-50000:]
    ...
    def __getitem__(self,index):
        ...
        label_tensor = t.zeros(25556).scatter_(0,t.LongTensor(labels),1).long()
        return data,label_tensor
...
class ZhihuALLData(data.Dataset):
    ...
    def __init__(self,train_root,labels_file,type_='char',augument=True):
        ...
        self.train_data = (all_char_title[:-50000],all_char_content[:-50000]),( all_word_title[:-50000],all_word_content[:-50000])
        self.val_data = (all_char_title[-50000:],all_char_content[-50000:]), (all_word_title[-50000:],all_word_content[-50000:])
