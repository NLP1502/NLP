...
def read_instance_substring(input_file, alphabet, substring_label_alphabet, number_normalized):
    ...
    for line in in_lines:
        if len(line.strip()) > 0:
            ...
            tockens = word.split()
            labels = label.split()
            tocken_Ids = map(alphabet.get_index, tockens)
            label_Ids = map(substring_label_alphabet.get_index, labels)
            instence_texts.append([tockens, labels])
            instence_Ids.append([tocken_Ids, label_Ids])
...
def load_pretrain_emb(embedding_path):
    ...
    with open(embedding_path, 'r') as file:
        for index, line in enumerate(file):
            if index == 0 and len(line.split()) < 5:
                continue
            else:
                line = line.strip()
                if len(line) == 0:
                ...
                if embedd_dim < 0:
                    ...
                    if embedd_dim + 1 != len(tokens):
                        print "wrong:", embedd_dim, len(tokens), index
