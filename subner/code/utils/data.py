...
class Data:
    ...
    def show_data_summary(self):
        ...
        print(" viterbi inputs model name: %s" % (self.viterbi_inputs_model_name))
    ...
    def initial_feature_alphabets(self):
        ...
        if self.feat_config:
            for idx in range(self.feature_num):
                self.feature_emb_dims[idx] = self.feat_config[self.feature_names[idx]]['emb_size']
                self.feature_emb_dirs[idx] = self.feat_config[self.feature_names[idx]]['emb_dir']
                self.norm_feature_embs[idx] = self.feat_config[self.feature_names[idx]]['emb_norm']
        # exit(0)
    ...
    def read_arg(self, args):
        ...
        if args.viterbi_inputs_model_name != None: self.viterbi_inputs_model_name = args.viterbi_inputs_model_name
        ...
        if args.circul_gather_output_mode != None: self.circul_gather_output_mode = args.circul_gather_output_mode

        ###decode_prepare
        if args.decode_prepare_mode != None: self.decode_prepare_mode = args.decode_prepare_mode
...
def init_parser(parser):
    ...
    parser.add_argument('--middle_dir')
    parser.add_argument('--viterbi_inputs_model_name')
    ...
    ###circul
    parser.add_argument('--circul_time', type=int)
    parser.add_argument('--circul_deepth', type=int)
    parser.add_argument('--circul_gather_output_mode', type=str)

    ###decode_prepare
    parser.add_argument('--decode_prepare_mode', type=str)
