...
full2halfconfig=os.path.join(cur_dir, "full_to_half.all")
    ...
    for line in configfile.read().split("\n"):
        if re.match("#punctuation", line):
            ...
            if  full == half:
                continue
            _dic[full] = half
    _dic[chr(8195)] = chr(32)
    _dic[chr(8194)] = chr(32)
    _dic[chr(12288)] = chr(32)
    _dic[chr(160)] = chr(32)
    for (full, half, end) in [(ord('ａ'), ord('a'), ord('ｚ')), \
                              (ord('Ａ'), ord('a'), ord('Ｚ')), \
                              (ord('０'), ord('0'), ord('９'))]:
        while full <= end:
            _dic[chr(full)] = chr(half)
            full += 1
            half += 1
