...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim):
        myTrack.show_memory_add("BiLSTM_CRF.__init__: begin")
        ...
        myTrack.show_memory_add("BiLSTM_CRF.__init__: step1")
        # Maps the output of the LSTM into tag space.
        self.hidden2tag = nn.Linear(hidden_dim, self.tagset_size)
        myTrack.show_memory_add("BiLSTM_CRF.__init__: step2")
        # Matrix of transition parameters.  Entry i,j is the score of
        # transitioning *to* i *from* j.
        self.transitions = nn.Parameter(
            torch.randn(self.tagset_size, self.tagset_size).to(device))
        myTrack.show_memory_add("BiLSTM_CRF.__init__: step3")
        # These two statements enforce the constraint that we never transfer
        # to the start tag and we never transfer from the stop tag
        self.transitions.data[tag_to_ix[START_TAG], :] = -10000
        self.transitions.data[:, tag_to_ix[STOP_TAG]] = -10000
        myTrack.show_memory_add("BiLSTM_CRF.__init__: step4")
        self.hidden = self.init_hidden()
        myTrack.show_memory_add("BiLSTM_CRF.__init__: end")
...
char2id, id2char, tag2id, id2tag = get_vocab("trainToy.src", "trainToy.tgt")
X, Y = get_dataset("trainToy.src", "trainToy.tgt", char2id, tag2id)
VX, VY = get_dataset("validToy.src", "validToy.tgt", char2id, tag2id)
...
for n, p in list(model.named_parameters()):
    ...
    print size
    total_size += size
print "total size: " + str(total_size*4/(1024**2)) + "M"

myTrack.show_memory_add("main: before model.to(device)")
model.to(device)
myTrack.show_memory_add("main: after model.to(device)")

optimizer = optim.SGD(model.parameters(), lr=0.01, weight_decay=1e-4)
myTrack.show_memory_add("main: after define optimizer")
