    ...
    for k, v in classifer_count_dic.items():
        ...
        def get_ans_list():
            ...
            for i in range(topN):
                ...
                if data.gpu:
                    model.cuda()
                model.load_state_dict(torch.load(os.path.join(data.model_dir, v[i][0])))
...
def make_classification_ans_ensemble_selected_models(data, name, tagNum=6):
    ...
    for k, v in ans_dict.items():
        feature_length = k[1]
        feature_ans_set = k[2]
        feature_ans_highest_dev_score = v[0][1]
...
if __name__ == '__main__':
    ...
    if status == 'train':
        ...
        out_predict, _ = make_classification_ans_ensemble_selected_models(data, 'test')
