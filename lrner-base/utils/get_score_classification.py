...
def get_score_topk(predict_label_and_marked_label_list, visible_top_num, num_classes):
    ...
    return (precision * recall) / (
                precision + recall + 0.0000000000001), precision, recall, right_label_at_pos_num, precision_classes_num
...
def get_score_one(predict_label_and_marked_label_list):
    pt = 0
    nt = 0
    pf = 0
    nf = 0
    ...
    p = pt / (pt + pf + b)
    r = pt / (pt + nt + b)
    a = (pt + nf) / (len(predict_label_and_marked_label_list) + b)
    f = 2 * p * r / (p + r + b)
    return  a, p, r, f
...
if __name__ == '__main__':
    ...
    a, b, c, d, e = get_score(predict_label_and_marked_label_list)
    print(a)
    print(b)
    print(c)
    print(d)
