...
class WordRep(nn.Module):
    def __init__(self, data):
        ...
        if self.use_trans:
            if self.use_mapping:
                # linear mapping
                self.w = nn.Linear(data.word_emb_dim, data.trans_hidden_dim)
            ...
            self.trans_hidden_dim = data.trans_hidden_dim
            ...
            self.trans_feature = TransBiLSTM(data.translation_alphabet.size(), self.trans_embedding_dim,
                                             ...
                                             data.dropout, data.pretrain_trans_embedding, self.gpu)
        ...
        if self.use_char:
            ...
            if data.char_seq_feature == "CNN":
                self.char_feature = CharCNN(data.char_alphabet.size(), self.char_embedding_dim, self.char_hidden_dim,
                                            data.dropout, data.pretrain_char_embedding, self.gpu)
                ...
                self.char_feature_extra = CharBiLSTM(data.char_alphabet.size(), self.char_embedding_dim,
                                                     self.char_hidden_dim, data.dropout, self.gpu)
            else:
