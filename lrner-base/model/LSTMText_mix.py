...
class LSTMText_mix(nn.Module):
    ...
    def __init__(self, data, feature_name, feature_ans, feature_length):
        ...
        if feature_name != 'word-pos':
            print('this version only deal feature:word-pos')

        if 'word' in feature_name:
            alphabet_size_word = data.word_alphabet.size()
            embedding_dim_word = data.word_emb_dim
        if 'pos' in feature_name:
            alphabet_size_pos = data.feature_alphabets[0].size()
            embedding_dim_pos = data.bpe_emb_dim

        self.input_size = embedding_dim_word + embedding_dim_pos

        self.word_embedding_word = nn.Embedding(alphabet_size_word, embedding_dim_word)
        self.word_embedding_pos = nn.Embedding(alphabet_size_pos, embedding_dim_pos)
        if data.pretrain_word_embedding is not None:
            self.word_embedding_word.weight.data.copy_(torch.from_numpy(data.pretrain_word_embedding))
        elif data.pretrain_feature_embeddings is not None:
            self.word_embedding_pos.weight.data.copy_(torch.from_numpy(data.pretrain_feature_embeddings[0]))
    ...
    def forward(self, word_inputs, word_seq_lengths, mask):
        word_represent_word = self.word_embedding_word(word_inputs[:,:,0])
        word_represent_pos = self.word_embedding_pos(word_inputs[:,:,1])
        word_represent = torch.cat([word_represent_word, word_represent_pos], -1)
