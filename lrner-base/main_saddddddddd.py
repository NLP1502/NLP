...
def select_ans_dict(ans_dict, feats_lists, maxlen=1):
    ans_dict_selected = []
    ...
    for k, v in ans_dict.items():
        ...
        for feats_list in feats_lists:
            num_correct += len(feats_list) - feature_length + 1
        if num_correct == num_feature_ans:
            print('{} len is {}'.format(k, num_correct))
            ...
            print('error in select_ans_dict: {}'.format(k))
    return ans_dict_selected, ans_dict_selected_info

def label_id_to_substring_label_id(data, k, out=[10,11]):
    if k == 0:
        return 0
    if k in out:
...
def get_id_list(data, score_path, cur_label, last_label, len):
    if len < 2:
        print 'len should bigger than 2'
        return
    id_list = []
    id_list.append(label_id_to_substring_label_id(data, cur_label))
    id_list.append(label_id_to_substring_label_id(data, last_label))
...
def id_list_in_ad_ans(id_list, ad_ans):
    flag = False
    for i in id_list:
        if i in ad_ans:
            flag = True
...
def reward(data, cur_label, num, ans_dict_selected, ans_dict_selected_info, ans_dict, feats_norm, pos, last_label, score_path):
    ## parameters
    ad_len_weight = [0, 1.0/30, 1.0/30, 1.0/30, 1.0/30, 1.0/30, 1.0/30, 1.0/30]
    adp_prf_weight_para = [1.0, 0, 0]
    ad_ans_num_weight_para = 1.0
    adp_scores_weight_para = [1.0, 0]

    id = label_id_to_substring_label_id(data, cur_label)
    reward_score = 0
    for ad_idx, ad in enumerate(ans_dict_selected):
        ...
        for adp_idx, adp in enumerate(v):
            ...
            ad_ans_num_weight = (1.0 / ad_ans_num) * ad_ans_num_weight_para
            ...
            if ad_len == 2:
                if pos+1 >= ad_len:
                    id_list = get_id_list(data, score_path, cur_label, last_label, 2)
                    if id_list_in_ad_ans(id_list, ad_ans):
                        adp_scores_weight = adp_scores[num[2]][1] * (feats_norm / ad_abs_mean) \
                                            * adp_scores_weight_para[0] + (adp_scores[num[2]][1] - adp_scores[num[2]][0]) \
                                            * (feats_norm / ad_abs_diff_mean) * adp_scores_weight_para[1]
                        reward_score += adp_scores_weight * adp_prf_weight * ad_ans_num_weight * ad_len_weight[2]

    return reward_score

def dynamic_planning_plus(data, feats_lists, inters, ans_dict):
    START_TAG = -2
    STOP_TAG = -1
    len_tag = 12
    ans_lists = []
    ...
    ans_dict_selected, ans_dict_selected_info = select_ans_dict(
        ans_dict, feats_lists, maxlen=2)
    ...
    for feats in feats_lists:
        ...
        score.append([inters[START_TAG][k] + feats[0][k] +
                      reward(data, k, num, ans_dict_selected, ans_dict_selected_info, ans_dict, norm[0], 0, -1, score_path)
            ...
            for k in range(len_tag):
                ...
                for j in range(len_tag):
                    stk.append(score[-1][j] + inters[j][k] + feats[i][k] +
                               reward(data, k, num, ans_dict_selected, ans_dict_selected_info, ans_dict, norm[i], i, j, score_path)
                      )
                stkmax = max(stk)
                st.append(stkmax)
...
def decode_prepare(data, name='test'):
    ...
    temp_start = time.time()
    ans_lists = dynamic_planning_plus(data, feats_lists, inters, ans_dict)
    temp_time = time.time()
    temp_cost = temp_time - temp_start
...
def classification_load_selected_models_and_decode(data, name, topN=1, rankin=['p','r','f']):
    ...
    print "Load Models from dir: ", outfiledir
    print "rankin: ", rankin
    with open(outfiledir + recordname, 'rb') as fpkl:
...
def classification_get_scores(data, name, topN=1, threshold='0.0'):
    ...
    f1 = file(name + '.score_dict.pkl', 'wb')
